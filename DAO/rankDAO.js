...
function rank(type, user_id) {
  return new Promise((resolve, reject) => {
    ...
    if (type == "all") {
      ...
      query = `SELECT u.user_id, u.name, u.total_score, u.profile_img FROM user u WHERE u.user_id IN (SELECT f.user_id2 from follow f WHERE f.user_id = ${user_id}) ORDER BY u.total_score DESC;`;
